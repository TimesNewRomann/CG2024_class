...
static const Cvec3 g_light1(5.0, 5.0, 5.0), g_light2(-7.0, -7.0, -7.0);  // define two lights positions in world space
...
void createStructure(const ShaderState& curSS, const Matrix4& transform, const Cvec3& baseColor) {
    // 왼쪽 벽
    auto leftPlane = createPlane(5.0, 10.0);
    drawPlane(curSS, leftPlane, transform * Matrix4::makeTranslation(Cvec3(-2.5, 0.5, -7.5)) * Matrix4::makeZRotation(90), baseColor);

    // 정면 벽
    auto facePlane = createPlane(5.0, 5.0);
    drawPlane(curSS, facePlane, transform * Matrix4::makeTranslation(Cvec3(0.0, 0.5, -12.5)) * Matrix4::makeXRotation(90), baseColor);

    // 오른쪽 벽
    auto rightPlane = createPlane(5.0, 10.0);
    drawPlane(curSS, rightPlane, transform * Matrix4::makeTranslation(Cvec3(2.5, 0.5, -7.5)) * Matrix4::makeZRotation(90), baseColor);
}
...
static void drawStuff() {
    ...
    //1동
    createStructure(curSS, Matrix4::makeTranslation(Cvec3(0.0, 0.0, 0.0)), Cvec3(0.99, 0.99, 0.99));

    //2동
    createStructure(curSS, Matrix4::makeTranslation(Cvec3(0.0, 0.0, 0.0)) * Matrix4::makeYRotation(-90), Cvec3(0.99, 0.99, 0.99));

    //3동
    //createStructure(curSS, Matrix4::makeTranslation(Cvec3(0.0, 0.0, 0.0)) * Matrix4::makeYRotation(180), Cvec3(0.1, 0.2, 0.5));
    // 3-3번 벽면
